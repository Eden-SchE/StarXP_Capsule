...
void MyG4BasedAnalysis::BeginOfRunAction()
{
    ...
    fCosmicRay = fCosmicRayList->GetGenerator();
    ...
    analysisManager->CreateNtuple("MCConfig", "MC Config Info"); // ntuple Id = 5
    analysisManager->CreateNtupleSColumn("PGType");
    analysisManager->CreateNtupleDColumn("GenTime");
    analysisManager->CreateNtupleSColumn("PGParameters");
    analysisManager->CreateNtupleIColumn("EvtNumber");

    analysisManager->CreateNtuple("MCTruth", "MCTruth Info"); // ntuple Id = 6
    analysisManager->CreateNtupleSColumn("Particle");
    analysisManager->CreateNtupleDColumn("TimeFlag");
    ...
    analysisManager->CreateNtupleDColumn("PolarizationX");
    analysisManager->CreateNtupleDColumn("PolarizationY");
    analysisManager->CreateNtupleDColumn("PolarizationZ");
...
void MyG4BasedAnalysis::EndOfRunAction()
{
    ...
    //Save MC Config Info
    PGPSConfig ConfigPS = fCosmicRayList->GetPGPSConfig();
    analysisManager->FillNtupleSColumn(5, 0, ConfigPS.ParticleGunType);
    analysisManager->FillNtupleDColumn(5, 1, ConfigPS.GenTime);
    ...
    analysisManager->FillNtupleSColumn(5, 2, G4String(PGParameters.substr(0, PGParameters.length()-1)));
    analysisManager->FillNtupleIColumn(5, 3, fCosmicRay->GetGenerateNumber());
...
void MyG4BasedAnalysis::EndOfEventAction(const G4Event *)
{
    ...
    G4ThreeVector Position = fCosmicRay->GetParticlePosition();
    G4ThreeVector Polarization = fCosmicRay->GetParticlePolarization();
    G4ThreeVector MomentumDirection = fCosmicRay->GetParticleMomentumDirection();

    analysisManager->FillNtupleSColumn(6, 0, fCosmicRay->GetParticle());
    analysisManager->FillNtupleDColumn(6, 1, fCosmicRay->GetTimeFlag());
    analysisManager->FillNtupleDColumn(6, 2, fCosmicRay->GetParticleEnergy());
    analysisManager->FillNtupleDColumn(6, 3, Position[0]);
    analysisManager->FillNtupleDColumn(6, 4, Position[1]);
    analysisManager->FillNtupleDColumn(6, 5, Position[2]);
    analysisManager->FillNtupleDColumn(6, 6, Polarization[0]);
    analysisManager->FillNtupleDColumn(6, 7, Polarization[1]);
    analysisManager->FillNtupleDColumn(6, 8, Polarization[2]);
    analysisManager->FillNtupleDColumn(6, 9, MomentumDirection[0]);
    analysisManager->FillNtupleDColumn(6, 10, MomentumDirection[1]);
    analysisManager->FillNtupleDColumn(6, 11, MomentumDirection[2]);
    analysisManager->AddNtupleRow(6);

    analysisManager->FillNtupleDColumn(3, 0, fTrkLen);
    analysisManager->FillNtupleDColumn(3, 1, fEng);
...
void MyG4BasedAnalysis::SteppingAction(const G4Step *aStep)
{
    ...
    if (parentID == 0)
    {
        ...
        double r = sqrt(postPos.x()*postPos.x() + postPos.y()*postPos.y());
        double z = postPos.z();

        if(r<250 && fabs(z)<204) fEng += aStep->GetTotalEnergyDeposit();
