...
G4double function_PrimaryCosmicRayProton(G4double *Eenergy,G4double *par){
...
PrimaryCosmicRayProton::PrimaryCosmicRayProton()
{
    fPGType = "Primary Cosmic Ray - Proton";
    fClassName = "PrimaryCosmicRayProton";
    fParticle = "proton";
...
void PrimaryCosmicRayProton::GenTimeVector()
{
    fTimeVector.clear();
    fEvtFlag = 0;

    TF1 *PCRpFunc = new TF1("PCRpFunc",function_PrimaryCosmicRayProton,5000,10000000,8);
    //A,a,solar_modulation,orbit_hight,Rearth,latitude,R,r
    PCRpFunc->SetParameters(A,a,solar_modulation,orbit_hight,Rearth,latitude,R,r);

    G4double uni_mean=PCRpFunc->Integral(bottom,top);//单位角度单位面积上的计数率
...
void PrimaryCosmicRayProton::Initialize(G4double GenTime, std::vector<std::string> PGParameters)
{
    if (PGParameters.size() > 0)
    {
        if(PGParameters.size() != 5)
        {
            G4cout << "Warning!!! Length of Input PGParameters and number of Initialize parameters of PrimaryCosmicRayProton not matched!!!" << G4endl;
...
void PrimaryCosmicRayProton::Generate()
{
    G4int success = 0;
    G4double energy = 0.;
    G4double latitude = 0.;
    ...
    TF1 *myPCRpFunc = new TF1("PCRpFunc",function_PrimaryCosmicRayProton,bottom,top,8);
    //A,a,solar_modulation,orbit_hight,Rearth,latitude,R,r
    myPCRpFunc->SetParameters(A,a,solar_modulation,orbit_hight,Rearth,latitude,R,r);
    energy=myPCRpFunc->GetRandom();
    delete myPCRpFunc;
