...
void CompareTrackL(){
    ...
    TH1F* histTrackEXP = new TH1F("Track Lenght Experiment","Track Lenght Experiment",30,0.,30);
    TH1F* histTrackMC = new TH1F("Track Lenght Simulation","Track Lenght Simulation",30,0.,30);
    TH1F* histTrackSimpMC = new TH1F("Track Lenght Simulation Simple Gaussain","Track Lenght Simulation Simple Gaussain",30,0.,30);
    ...
    for(Long64_t i = 0; i < entriesMC; i ++)
    {
        ...
        if(sumADC>sumADCCut) continue;
        // if(sumADC<sumADCCutMin) continue;
        histTrackSimpMC->Fill(length);
        MCSimpNumber+=1.;
    }

    histTrackEXP->SetMarkerStyle(20);
    histTrackEXP->SetMarkerColor(kRed);
    histTrackEXP->SetMarkerSize(1);
    histTrackMC->SetMarkerStyle(22);
    histTrackMC->SetMarkerColor(kBlue);
    histTrackMC->SetMarkerSize(1);
    histTrackSimpMC->SetLineWidth(1);
    histTrackSimpMC->SetLineColor(kGreen);
    // can->cd(1);
    histTrackEXP->Scale(1./double(EXPNum));
    histTrackMC->Scale(1./double(MCNumber));
    histTrackSimpMC->Scale(1./double(MCSimpNumber));

    histTrackSimpMC->SetFillStyle(3005);
    histTrackSimpMC->SetFillColor(kGreen);
    histTrackEXP->SetLineColor(kRed);
    histTrackEXP->SetFillStyle(3003);
    histTrackMC->SetFillStyle(3004);
    histTrackEXP->SetFillColor(kRed);
    histTrackMC->SetFillColor(kBlue);
    // histTrackEXP->GetYaxis()->SetRangeUser(0,0.052);
    // histTrackMC->GetYaxis()->SetRangeUser(0,0.052);
    histTrackEXP->SetStats(kFALSE);
    histTrackMC->SetStats(kFALSE);
    // histTrackMC->GetXaxis()->SetTitle("Track Lenght");
    // histTrackMC->GetYaxis()->SetTitle("Counts/Entries");
    TAxis *xAxis = histTrackMC->GetXaxis();
    ...
    TAxis *yAxis = histTrackMC->GetYaxis();
    ...
    histTrackEXP->Draw("HIST E0");
    histTrackMC->Draw("SAME HIST");
    // histTrackSimpMC->Draw("SAME HIST");
    TLegend* leg = can->BuildLegend();
    leg->SetBorderSize(0);
    leg->SetTextSize(0.03);
    latex->DrawLatexNDC(0.5, 0.025, "Track Lenght[pixel]");
    latey->DrawLatexNDC(0.025, 0.5, "Normalized entries");
    std::cout<<"Avr Lenght is:"<<sumlenght/MCNumber<<std::endl;
    std::cout<<"Avr Lenght is:"<<sumlenghtEXP/EXPNum<<std::endl;
